      ...
      {tab === "card" && <>
        <div className="grid items-stretch gap-5 lg:grid-cols-[1.35fr_1fr] lg:gap-7">
          <section className={cn("relative isolate flex min-h-[304px] flex-col overflow-hidden rounded-[24px] bg-[#153e34] p-6 text-[#f6f7ed] shadow-[0_12px_32px_-18px_rgba(20,61,50,0.5)] sm:min-h-[328px] sm:p-8", member.tier === "gold" && "bg-[#244537]")} aria-label={t("card")}>
            ...
            <div className="flex items-start justify-between gap-3"><div><BrandMark className="text-[24px]" accentClassName="text-[#c7d49f]" /><p className="mt-1 text-[9px] tracking-[0.07em] text-[#d4dfd3]/75">Sing Spare Parts Co. Pte Ltd</p></div><span className="inline-flex items-center gap-1.5 rounded-full border border-[#d6d5ae]/40 px-3 py-1.5 text-[10px] font-medium tracking-wide text-[#e0ddb1]"><Sparkles className="size-3" />{t("membershipTier", { tier: t(member.tier) })}</span></div>
            <div className="mt-9"><p className="text-[11px] text-[#c9d7cc]/75">{t("availablePoints")}</p><div className="mt-1.5 flex items-baseline gap-2"><span key={member.points} className={cn("inline-block origin-bottom-left text-[64px] font-light leading-none tracking-[-0.055em] tabular-nums sm:text-[72px]", feedback?.kind === "purchase" && feedback.points > 0 && "motion-safe:animate-[points-settle_320ms_ease-out]")}>{member.points}</span><span className="text-xs text-[#ceddce]">{t("pointUnit", { count: member.points })}</span></div></div>
