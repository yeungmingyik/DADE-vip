...
export function MemberApp() {
  ...
  if (!data || !member) {
    return <div className="min-h-screen bg-background"><Topbar role="member" /><main className="mx-auto flex min-h-[60vh] max-w-xl flex-col items-center justify-center gap-5 px-6 text-center"><CreditCard className="size-10 text-primary" /><p className="text-sm text-muted-foreground">{loading ? t("loading") : error || t("noSelection")}</p>{!loading && <Button variant="outline" onClick={() => void refresh()}>{t("retry")}</Button>}</main></div>;
  }
  ...
  const activityRow = (item: Activity) => {
    ...
    return <button key={item.id} onClick={() => setSelectedActivity(item)} className="group flex min-h-20 w-full items-center gap-3.5 border-b border-black/5 px-1 py-4 text-left last:border-b-0 focus-visible:outline-2 focus-visible:outline-primary">
      <span className={cn("flex size-10 shrink-0 items-center justify-center rounded-full", positive ? "bg-success text-success-foreground" : "bg-stone-100 text-stone-600")}><Icon className="size-[18px]" /></span>
      <span className="min-w-0 flex-1"><span className="block truncate text-sm font-medium text-foreground">{t(item.kind)}</span><span className="mt-1 block truncate text-xs text-muted-foreground">{storeName(item.storeId)} <span className="px-1.5 text-stone-300">·</span> {dateTime(item.createdAt, locale)}</span></span>
      <span className="shrink-0 text-right"><span className={cn("block text-sm font-semibold tabular-nums", positive ? "text-success-foreground" : "text-foreground")}>{positive ? "+" : ""}{item.pointsDelta} <span className="text-[10px] font-medium">{t("pointUnit", { count: Math.abs(item.pointsDelta) })}</span></span>{item.amountCents !== undefined && <span className="mt-1 block text-xs tabular-nums text-muted-foreground">{money(item.amountCents, locale)}</span>}</span>
  ...
  const giftCard = (gift: Gift, compact = false) => <button key={gift.id} onClick={() => setSelectedGift(gift)} className={cn("group min-w-0 overflow-hidden rounded-2xl border border-black/5 bg-white text-left transition duration-150 hover:border-primary/25 hover:shadow-sm focus-visible:outline-2 focus-visible:outline-primary", compact ? "grid grid-cols-[112px_minmax(0,1fr)] sm:grid-cols-[140px_minmax(0,1fr)]" : "flex flex-col")}>
    <div className={cn("relative overflow-hidden bg-muted", compact ? "min-h-36" : "aspect-[4/3]")}><Image src={gift.image} alt={localText(gift.name, locale)} fill sizes={compact ? "140px" : "(max-width: 768px) 50vw, 320px"} className="object-cover transition-transform duration-300 motion-safe:group-hover:scale-[1.035]" /></div>
    <div className={cn("flex min-w-0 flex-1 flex-col [overflow-wrap:anywhere]", compact ? "justify-center p-4" : "p-4 sm:p-5")}><span className="mb-2 text-[10px] font-medium uppercase tracking-[0.15em] text-stone-500">{t(gift.category)}</span><h3 className="font-medium leading-snug text-[#501b24]">{localText(gift.name, locale)}</h3><div className="mt-3 flex flex-wrap items-center justify-between gap-2"><span className="text-sm font-semibold text-primary">{t("rewardCost", { points: gift.points })}</span><ArrowUpRight className="size-4 text-primary" /></div>{!compact && <span className={cn("mt-3 text-xs", giftState(gift) === "available" ? "text-success-foreground" : "text-stone-500")}>{t(giftState(gift))}</span>}</div>
  </button>;
  ...
  return <div className="min-h-screen bg-background text-foreground">
    ...
    <main className="mx-auto max-w-[1180px] px-4 pb-28 pt-7 sm:px-7 md:pb-14 md:pt-10 lg:px-10">
      ...
      {feedback && <div key={feedback.sequence} role="status" className="fixed inset-x-4 bottom-[calc(5.5rem+env(safe-area-inset-bottom))] z-40 mx-auto flex max-w-lg items-center gap-3 rounded-2xl border border-success-border bg-success px-4 py-3 text-success-foreground shadow-[0_12px_36px_-12px_rgba(40,35,38,0.18)] motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-bottom-1 md:bottom-6"><span className="flex size-9 shrink-0 items-center justify-center rounded-full bg-white"><CircleCheck className="size-5 text-success-foreground" /></span><div className="flex-1 text-sm"><p className="font-semibold">{feedback.tier ? t("newTier", { tier: t(feedback.tier) }) : feedback.aggregated ? t("activityUpdated") : feedback.count > 1 ? t("newActivity", { count: feedback.count }) : feedback.kind === "purchase" && feedback.points > 0 ? t("pointsReceived") : t(feedback.kind)}</p><button onClick={() => switchTab("activity")} className="mt-0.5 text-xs underline underline-offset-4">{t("viewActivity")}</button></div>{feedback.points !== 0 && <span className="font-semibold tabular-nums">{feedback.points > 0 ? "+" : ""}{feedback.points}</span>}<button aria-label={t("close")} onClick={() => setFeedback(null)} className="flex size-9 items-center justify-center rounded-full hover:bg-success-foreground/8"><X className="size-4" /></button></div>}

      {tab === "card" && <>
        <div className="grid grid-cols-1 items-stretch gap-5 lg:grid-cols-[minmax(0,1.35fr)_minmax(0,1fr)] lg:gap-7">
          <section className={cn("relative isolate flex min-h-[304px] flex-col overflow-hidden rounded-[24px] bg-[#501b24] p-6 text-[#fffaf5] shadow-[0_12px_32px_-18px_rgba(80,27,36,0.3)] sm:min-h-[328px] sm:p-8", member.tier === "gold" && "bg-[#5c252b]")} aria-label={t("card")}>
            <div aria-hidden="true" className="pointer-events-none absolute -right-28 -top-32 -z-10 size-[480px] rounded-full border border-[#d9c6b2]/[0.09]" /><div aria-hidden="true" className="pointer-events-none absolute -right-8 -top-12 -z-10 size-[320px] rounded-full border border-[#d9c6b2]/[0.12]" /><div aria-hidden="true" className="pointer-events-none absolute -right-24 -top-28 -z-10 size-[350px] rounded-full bg-[#e5d6c5]/[0.10]" />
            ...
            <div className="mt-9"><p className="text-[11px] text-[#e3d5cd]">{t("availablePoints")}</p><div className="mt-1.5 flex items-baseline gap-2"><span key={member.points} className={cn("inline-block origin-bottom-left text-[64px] font-light leading-none tracking-[-0.055em] tabular-nums sm:text-[72px]", feedback?.kind === "purchase" && feedback.points > 0 && "motion-safe:animate-[points-settle_320ms_ease-out]")}>{member.points}</span><span className="text-xs text-[#e3d5cd]">{t("pointUnit", { count: member.points })}</span></div></div>
            <div className="mt-auto flex items-end justify-between gap-4 pt-8"><div><p className="text-sm font-medium tracking-wide">{member.name}</p><p className="mt-1.5 font-mono text-[10px] tracking-[0.2em] text-[#d8c7bd]">{member.number}</p></div><button onClick={() => setCodeOpen(true)} aria-label={t("showCode")} className="flex size-[58px] shrink-0 items-center justify-center rounded-xl bg-[#fffaf5] text-[#501b24] transition-transform hover:bg-white active:scale-95"><QrCode className="size-8 stroke-[1.6]" /></button></div>
          </section>
          ...
          <section className="flex flex-col rounded-[24px] border border-stone-200/70 bg-white p-6 sm:p-7" aria-label={t("monthlyVisits")}>
            ...
            <div className="mt-auto flex items-center gap-3 border-t border-stone-100 pt-5 max-lg:mt-5"><span className="flex size-9 shrink-0 items-center justify-center rounded-full bg-muted text-primary"><GiftIcon className="size-4" /></span><div className="flex-1"><p className="text-xs font-medium">{t("rewardAllowance")}</p><p className="mt-1 text-[11px] text-stone-500">{t("quota", { remaining: quotaRemaining, total: data.rules.monthlyLimit })}</p></div><button onClick={() => switchTab("rewards")} aria-label={t("viewAll")} className="flex size-10 items-center justify-center rounded-full hover:bg-stone-50"><ArrowRight className="size-4" /></button></div>
          </section>
        </div>
      ...
      {tab === "account" && <section className="mx-auto max-w-2xl overflow-hidden rounded-[24px] border border-stone-200/70 bg-white"><div className="flex items-center gap-5 border-b border-stone-100 p-6 sm:p-8"><div className="flex size-16 items-center justify-center rounded-full bg-muted text-xl font-medium text-foreground">{member.name.split(" ").map((part) => part[0]).slice(0, 2).join("")}</div><div><h2 className="text-xl font-medium">{member.name}</h2><p className="mt-2 text-xs text-stone-500">{t(member.tier)} · {member.number}</p></div><Badge className="ml-auto" variant="outline">{t(member.status)}</Badge></div><dl className="divide-y divide-stone-100 px-6 sm:px-8">{[[t("memberNumber"), member.number], [t("phone"), member.phone], [t("joined"), dateTime(member.joinedAt, locale)], [t("totalSpend"), money(member.totalSpendCents, locale)], [t("availablePoints"), String(member.points)]].map(([label, value]) => <div key={label} className="flex flex-wrap items-center justify-between gap-3 py-5 text-sm"><dt className="text-stone-500">{label}</dt><dd className="font-medium">{value}</dd></div>)}</dl><div className="p-6 pt-3 sm:px-8"><Button onClick={() => setCodeOpen(true)} className="h-11 w-full rounded-full bg-primary"><QrCode className="size-4" />{t("showCode")}</Button></div></section>}
    </main>

    <nav aria-label={t("membership")} className="fixed inset-x-0 bottom-0 z-30 grid grid-cols-4 border-t border-stone-200/80 bg-[#fffdfa] px-3 pb-[max(env(safe-area-inset-bottom),8px)] pt-2 md:hidden">{navigation.map(({ id, icon: Icon }) => <button key={id} onClick={() => switchTab(id)} aria-current={tab === id ? "page" : undefined} className={cn("flex min-h-[56px] flex-col items-center justify-center gap-1.5 rounded-xl text-[10px] font-medium", tab === id ? "text-primary" : "text-stone-400")}><span className={cn("rounded-full px-5 py-1", tab === id && "bg-muted")}><Icon className="size-5" strokeWidth={tab === id ? 2 : 1.6} /></span>{t(id)}</button>)}</nav>
