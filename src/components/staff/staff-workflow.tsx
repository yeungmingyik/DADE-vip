...
export function StaffApp() {
  ...
  if (!data) return <div className="min-h-screen bg-[#f7f8fa]"><Topbar role="staff" /><main className="mx-auto flex min-h-[60vh] max-w-xl flex-col items-center justify-center gap-5 px-6 text-center"><ScanLine className="size-10 text-emerald-800" /><p className="text-sm text-muted-foreground">{loading ? t("loading") : error}</p>{!loading && <Button variant="outline" onClick={() => void refresh()}>{t("retry")}</Button>}</main></div>;
  ...
  const memberSummary = (selected: Member, editable = false) => <div className="flex flex-wrap items-center gap-3 rounded-xl border border-stone-200 bg-[#f8f9f6] p-4"><span className="flex size-11 shrink-0 items-center justify-center rounded-full bg-[#e8eee2] text-sm font-semibold text-emerald-900">{initials(selected.name)}</span><div className="min-w-0 flex-1"><p className="font-medium">{selected.name}</p><p className="mt-1 text-xs text-stone-500">{selected.number}</p></div><Badge variant="outline" className="border-[#d9d4b4] text-[#857642]">{t(selected.tier)}</Badge>{editable && <button className="min-h-10 text-xs font-medium text-emerald-800" onClick={changeMember} disabled={busy}>{t("changeMember")}</button>}</div>;
  const purchaseRows = (record: Purchase): [string, string][] => [[t("member"), record.memberName], [t("receipt"), record.receipt], [t("store"), storeName(record.storeId)], [t("recordedAt"), dateTime(record.createdAt, locale)], [t("points"), String(record.points)], [t("refunded"), money(record.refundedCents, locale)]];
