...
function receiptNumber() { return `DADE-${crypto.randomUUID().slice(0, 8).toUpperCase()}`; }
...
export function StaffApp() {
  ...
  return <div className="min-h-screen bg-[#f8f6f6] text-[#242126]">
    ...
    <Dialog open={recordDialog !== null} onOpenChange={(open) => { if (!open && !busy) { setRecordDialog(null); setNotice(null); setLocalError(null); } }}><DialogContent className="max-h-[90dvh] max-w-[460px] overflow-y-auto rounded-2xl p-6">{recordDialog && <>
      ...
      {notice && <div role="status" className="flex items-center gap-2 rounded-xl border border-success-border bg-success p-3 text-sm font-medium text-success-foreground"><CheckCircle2 className="size-4" />{t(notice)}</div>}
