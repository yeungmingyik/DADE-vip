...
export function AdminApp() {
  ...
  const overview = data && <div className="space-y-6">
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{[
      { title: "totalMembers", value: data.stats.members.toLocaleString(locale), sub: `${data.stats.activeMembers} ${t("activeMembers")}`, icon: Users },
      { title: "registeredSpend", value: money(data.stats.salesCents, locale), sub: `${data.stats.purchaseCount} ${t("transactions")}`, icon: CircleDollarSign },
      { title: "pointsIssued", value: data.stats.pointsIssued.toLocaleString(locale), sub: t("points"), icon: ArrowUpRight },
      { title: "rewardsRedeemed", value: data.stats.redemptionCount.toLocaleString(locale), sub: `${data.stats.pendingRedemptions} ${t("pendingHandover")}`, icon: GiftIcon }
