...
import { ArrowDownToLine, ArrowRight, ArrowUpRight, Check, ChevronLeft, ChevronRight, CircleDollarSign, Ellipsis, Gift as GiftIcon, LayoutDashboard, Loader2, Menu, Package, ReceiptText, RefreshCw, Search, Settings2, ShieldCheck, Store as StoreIcon, Users } from "lucide-react";
...
function ActivityChart({ data, locale, label }: { data: AppData["series"]; locale: string; label: string }) {
  ...
  return <div className="mt-5"><svg role="img" aria-label={label} viewBox="0 0 730 225" className="w-full overflow-visible"><defs><linearGradient id={id} x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#bd202b" stopOpacity=".16" /><stop offset="100%" stopColor="#bd202b" stopOpacity=".01" /></linearGradient></defs>{[0, 1, 2, 3].map(index => <g key={index}><line x1="44" x2="702" y1={43 + index * 47.3} y2={43 + index * 47.3} stroke="#eee7e8" strokeDasharray="3 5" /><text x="0" y={47 + index * 47.3} fill="#81787d" fontSize="10">{Math.round(max * (1 - index / 3) / 100)}</text></g>)}<path d={`${line} L694,185 L44,185 Z`} fill={`url(#${id})`} /><path d={line} fill="none" stroke="#bd202b" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />{data.filter((_, index) => index === 0 || index === data.length - 1 || index === Math.floor(data.length / 2)).map(point => { const index = data.indexOf(point); return <text key={point.date} x={44 + index * 650 / Math.max(1, data.length - 1)} y="216" textAnchor={index === 0 ? "start" : index === data.length - 1 ? "end" : "middle"} fill="#81787d" fontSize="11">{new Intl.DateTimeFormat(locale, { day: "numeric", month: "short", timeZone: "Asia/Singapore" }).format(new Date(point.date))}</text>; })}</svg></div>;
}
...
export function AdminApp() {
  ...
  const statusBadge = (status: string) => <Badge variant="outline" className={cn("px-2 py-1 text-[10px] font-medium", ["active", "fulfilled", "registered"].includes(status) ? "border-success-border bg-success text-success-foreground" : ["confirmed", "partiallyRefunded"].includes(status) ? "border-amber-200 bg-amber-50 text-amber-800" : "border-stone-200 bg-stone-100 text-stone-600")}>{["active", "fulfilled", "registered"].includes(status) && <Check aria-hidden="true" className="size-3" />}{t.has(status) ? t(status) : status}</Badge>;
  const tierBadge = (tier: string) => <Badge variant="outline" className={cn("font-medium text-[10px]", tier === "gold" ? "border-[#e7d9b4] bg-[#fbf6e6] text-[#8a6c27]" : tier === "silver" ? "border-[#ddd8dd] bg-[#f3f1f3] text-[#655f67]" : "border-[#ead7c4] bg-[#faf2e9] text-[#8d664b]")}>{t(tier)}</Badge>;

  const nav = <><div className="px-5 pb-5 pt-6"><p className="text-[9px] font-semibold tracking-[.22em] text-[#b6a8ae]">{t("workspace")}</p><div className="mt-3 flex items-center gap-2.5"><BrandMark onDark /><span className="rounded border border-white/15 px-1.5 py-1 text-[9px] font-semibold tracking-[0.14em] text-[#e8dce0]">VIP</span></div></div><nav className="space-y-1 px-3" aria-label={t("administration")}>{navigation.map(({ key, icon: Icon }, index) => <div key={key} className={index === 5 ? "mt-8! border-t border-white/10 pt-6" : ""}><button onClick={() => changeSection(key)} aria-current={section === key ? "page" : undefined} className={cn("flex min-h-11 w-full items-center gap-3 rounded-xl px-4 text-sm transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#f2a3ac]", section === key ? "bg-[#fbe6e8] font-semibold text-[#8f1825]" : "text-[#d4c9ce] hover:bg-white/7 hover:text-white")}><Icon className="size-[17px]" />{t(key)}{key === "redemptions" && !!data?.stats.pendingRedemptions && <span className={cn("ml-auto rounded-md px-1.5 text-[10px]", section === key ? "bg-[#8f1825]/10" : "bg-white/10")}>{data.stats.pendingRedemptions}</span>}</button></div>)}</nav><div className="mt-auto p-5"><div className="flex items-center gap-3 border-t border-white/10 pt-5"><div className="grid size-9 place-items-center rounded-full bg-[#bd202b] text-xs font-semibold text-white">HQ</div><div><p className="text-xs font-medium text-white">{data?.session.name || BRAND_NAME}</p><p className="mt-1 text-[10px] text-[#b6a8ae]">{t("headquarters")}</p></div></div></div></>;

  function purchaseTable(rows: Purchase[]) { return <Table><TableHeader><TableRow><TableHead>{t("member")}</TableHead><TableHead>{t("receipt")}</TableHead><TableHead>{t("store")}</TableHead><TableHead className="text-right">{t("amount")}</TableHead><TableHead className="text-right">{t("earned")}</TableHead><TableHead>{t("date")}</TableHead><TableHead>{t("status")}</TableHead>{section === "purchases" && <TableHead className="text-right">{t("actions")}</TableHead>}</TableRow></TableHeader><TableBody>{rows.map(row => <TableRow key={row.id}><TableCell><span className="flex items-center gap-2.5"><span className="grid size-8 shrink-0 place-items-center rounded-full bg-[#fce9eb] text-[10px] font-semibold text-[#8f1825]">{initials(row.memberName)}</span><span className="font-medium">{row.memberName}</span></span></TableCell><TableCell className="font-mono text-[10px] text-muted-foreground">{row.receipt}</TableCell><TableCell>{storeName(row.storeId)}</TableCell><TableCell className="text-right font-medium tabular-nums">{money(row.amountCents, locale)}</TableCell><TableCell className="text-right text-success-foreground">+{row.points}</TableCell><TableCell className="whitespace-nowrap text-muted-foreground">{dateTime(row.createdAt, locale)}</TableCell><TableCell>{statusBadge(row.refundedCents === 0 ? "registered" : row.refundedCents < row.amountCents ? "partiallyRefunded" : "refunded")}</TableCell>{section === "purchases" && <TableCell className="text-right"><Button size="sm" variant="ghost" disabled={row.refundedCents >= row.amountCents} onClick={() => open({ type: "refund", value: row })}>{t("refund")}</Button></TableCell>}</TableRow>)}</TableBody></Table>; }
  ...
  const overview = data && <div className="space-y-6">
    ...
    ].map(({ title, value, sub, icon: Icon }) => <Card key={title} className="gap-0 border-[#eae2e4] py-5 shadow-none"><CardContent className="px-5"><div className="flex items-center justify-between"><p className="text-xs font-medium text-muted-foreground">{t(title)}</p><Icon className="size-4 text-[#bd202b]" /></div><p className="mt-5 text-[29px] font-semibold tracking-[-.04em] tabular-nums">{value}</p><p className="mt-2 text-[11px] text-muted-foreground">{sub}</p></CardContent></Card>)}</div>
    <div className="grid gap-5 xl:grid-cols-[1.9fr_1fr]"><Card className="gap-0 shadow-none"><CardHeader className="flex flex-row items-start justify-between pb-0"><div><CardTitle className="text-base">{t("purchaseActivity")}</CardTitle><p className="mt-1.5 text-xs text-muted-foreground">{t("purchaseActivitySub")}</p></div><span className="rounded-md bg-[#fce9eb] px-2 py-1 text-[10px] font-medium text-[#8f1825]">{data.series.length ? `${new Intl.DateTimeFormat(locale, {month:"short",day:"numeric"}).format(new Date(data.series[0].date))} – ${new Intl.DateTimeFormat(locale, {month:"short",day:"numeric"}).format(new Date(data.series[data.series.length-1].date))}` : t(period)}</span></CardHeader><CardContent><ActivityChart data={data.series} locale={locale} label={t("purchaseActivity")} /></CardContent></Card><Card className="shadow-none"><CardHeader><CardTitle className="text-base">{t("membershipMix")}</CardTitle></CardHeader><CardContent className="space-y-6">{(["gold", "silver", "bronze"] as const).map((tier, index) => { const count = data.stats.tierCounts[tier]; const percentage = data.stats.members ? Math.round(count / data.stats.members * 100) : 0; return <div key={tier}><div className="mb-2 flex items-center justify-between text-xs"><span className="flex items-center gap-2"><span className={cn("size-2 rounded-full", index === 0 ? "bg-[#b49a53]" : index === 1 ? "bg-[#9a969b]" : "bg-[#ad7a56]")} />{t(tier)}</span><span className="tabular-nums text-muted-foreground">{count} <span className="ml-2 text-[10px]">{percentage}%</span></span></div><div className="h-2 overflow-hidden rounded-full bg-[#f2edef]"><div className={cn("h-full rounded-full", index === 0 ? "bg-[#bba66d]" : index === 1 ? "bg-[#b6b2b7]" : "bg-[#bd916e]")} style={{ width: `${percentage}%` }} /></div></div>; })}</CardContent></Card></div>
  ...
  return <div className="admin-shell grid h-dvh grid-rows-[auto_minmax(0,1fr)] overflow-hidden bg-[#f8f6f6] text-[#242126]"><Topbar role="admin" /><div className="flex min-h-0 overflow-hidden"><aside className="hidden h-full w-[216px] shrink-0 flex-col overflow-y-auto overscroll-none bg-[#242126] lg:flex">{nav}</aside><Dialog open={menu} onOpenChange={setMenu}><DialogContent className="inset-y-0 left-0 top-0 h-dvh max-w-[280px] translate-x-0 translate-y-0 overflow-y-auto overscroll-none rounded-none border-0 bg-[#242126] p-0 text-white"><DialogHeader className="sr-only"><DialogTitle>{t("menu")}</DialogTitle><DialogDescription>{t("workspace")}</DialogDescription></DialogHeader><div className="flex min-h-full flex-col">{nav}</div></DialogContent></Dialog>
    ...
    {modal?.type === "member" ? <div className="space-y-5"><div className="flex items-center gap-4"><span className="grid size-14 place-items-center rounded-full bg-[#fce9eb] text-lg font-semibold">{initials(modal.value.name)}</span><div><h3 className="text-lg font-semibold">{modal.value.name}</h3><div className="mt-2 flex gap-2">{tierBadge(modal.value.tier)}{statusBadge(modal.value.status)}</div></div></div><div className="grid grid-cols-2 gap-4 rounded-xl bg-muted p-5">{[{ label: "balance", value: modal.value.points }, { label: "visits", value: modal.value.visits }, { label: "totalSpend", value: money(modal.value.totalSpendCents, locale) }, { label: "phone", value: modal.value.phone }].map(item => <div key={item.label}><p className="text-[10px] text-muted-foreground">{t(item.label)}</p><p className="mt-1.5 text-sm font-semibold">{item.value}</p></div>)}</div><Button disabled={busy} variant={modal.value.status === "active" ? "outline" : "default"} className="w-full" onClick={async () => { if (await mutate({ action: "updateMember", memberId: modal.value.id, status: modal.value.status === "active" ? "suspended" : "active" })) setModal(null); }}>{t(modal.value.status === "active" ? "suspend" : "reactivate")}</Button></div> : <form onSubmit={submitModal} className="space-y-5">
      ...
      {modal?.type === "staff" && <><p className="text-lg font-semibold">{modal.value.name}</p><label className="block text-sm">{t("role")}<select name="role" className="field-select mt-2 w-full" defaultValue={modal.value.role}><option value="cashier">{t("cashier")}</option><option value="supervisor">{t("supervisor")}</option></select></label><fieldset className="space-y-3"><legend className="mb-3 text-sm font-medium">{t("access")}</legend>{data?.stores.filter(store => !store.deleted).map(store => <label className="flex items-center gap-3 text-sm" key={store.id}><input className="size-4 accent-[#bd202b]" type="checkbox" name="stores" value={store.id} defaultChecked={modal.value.storeIds.includes(store.id)} />{localText(store.name, locale)}</label>)}</fieldset><label className="flex items-center gap-3 border-t pt-4 text-sm"><input className="size-4 accent-[#bd202b]" name="active" type="checkbox" defaultChecked={modal.value.active} />{t("accountActive")}</label></>}
